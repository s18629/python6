...
@click.pass_context
def run_command(ctx):
    ctx.obj = {"db": database.get_database()}
...
@db_command.command("initialize-alchemy")
@click.pass_obj
def initialize_command(obj):
    database.create_db()
...
def create_command(obj, room_password, name):
    with obj["db"]:
        rooms_service.create_room(
            obj["db"].cursor(), obj["user"].id, room_password, name
        )
...
def join_room_command(obj, room_id, room_password):
    with obj["db"]:
        if not rooms_service.join_room(
                obj["db"].cursor(), obj["user"].id, room_id, room_password
        ):
